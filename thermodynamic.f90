module thermodynamic
  ...
  real*8 :: nu_i(3), gas_integrand(3), solid_integrand(3), dos_subinterval(3), solid_weight(3), quadrature(3), ener_weight(3)
  real*8, allocatable :: SHSbykB_group(:,:), SHSbykB_supergroup(:,:), SRbykB_group(:), SRbykB_supergroup(:)
  real*8, allocatable :: entropy_group(:,:), entropy_supergroup(:,:), entropy_supergroup_gas(:,:), ener_group(:,:)
...
subroutine get_entropy()
  ...
  allocate( ener_group(1:ngroups, 1:3) )
...
! For groups
  ...
  open(unit=60, file="Helmholtz_energy", status="unknown")
  ...
  write(30,*)"# Group; Energies in eV/K [trans, rot, vib]; Total energy in eV/K; Total energy in J/K"
  do j=1,ngroups
    do k=1,3
...
!       Calculate gas entropy
        if(k == 1 .or. k == 3)then
          ...
          ener_group(j,k) = ener_group(j,k) + dot_product(gas_integrand,quadrature) * (0.50 - SHSbykB_group(j,k) / 3.d0)
        else if(k == 2)then
          entropy_group(j,k) = entropy_group(j,k) + dot_product(gas_integrand,quadrature) * SRbykB_group(j) / 3.d0
          ener_group(j,k) = ener_group(j,k) + dot_product(gas_integrand,quadrature) * (0.50 - SRbykB_group(j) / 3.d0)
        end if
!
!       Solid part
        solid_weight = h * nu_i / kB / T
        ener_weight = dlog(1.d0 - dexp(-solid_weight)) + solid_weight / 2.d0
...
!       Add solid part if the remaining DoS is positive
        ...
        do j2=1,3
          if(solid_integrand(j2) > 0.d0)then
            ...
            ener_group(j,k) = ener_group(j,k) + solid_integrand(j2) * ener_weight(j2) * quadrature(j2)
          end if
        end do
!       Calculate 1PT part
      ...
      ener_group(j,k) = ener_group(j,k) * kB * T / tau
...
!   Entropy of mixture
    ...
    temp(5) = ener_group(j,1) + ener_group(j,2) + ener_group(j,3)
    write(60,'(I8,2X,ES15.8,2X,ES15.8,2X,ES15.8,2X,ES15.8,2X,ES15.8)') j, ener_group(j,1:3), temp(5), temp(5) * eVtoJ
  end do
! For supergroups
  if(calc_supergroups)then
